...
RECENTER_ATR_MULT   = float(os.getenv("RECENTER_ATR_MULT", "1.5"))  # rebuild als prijs te ver van center wegdrijft (in ATR)
MAX_CENTER_AGE_SEC  = int(os.getenv("MAX_CENTER_AGE_SEC", "900"))        # rebuild na max leeftijd van grid-center (sec)
...
def paper_place_tp_long(qty, entry_price, spacing, center):
    ...
    ref_ts = paper["open_entries"][-1]["ts"] if paper["open_entries"] else None
    paper["open_orders"].append({"id": oid, "type": "tp", "side": "sell", "price": tp, "qty": qty, "status": "open", "ref_ts": ref_ts, "ref_entry": entry_price})
    if NOTIFY_TP_PLACED:
...
def paper_place_tp_short(qty, entry_price, spacing, center):
    ...
    ref_ts = paper["short_entries"][-1]["ts"] if paper["short_entries"] else None
    paper["open_orders"].append({"id": oid, "type": "tp", "side": "buy", "price": tp, "qty": qty, "status": "open", "ref_ts": ref_ts, "ref_entry": entry_price})
    if NOTIFY_TP_PLACED:
        send_telegram(f"🧪 PAPER TP geplaatst BUY @ {tp:.6f} (short)")


def _take_by_ts(lots_key, ref_ts, qty_need):
    """Neem (deel van) lots met een specifieke ts (lot) om TP aan de juiste entry te koppelen."""
    if ref_ts is None:
        return [], 0.0
    taken = []
    qty_left = qty_need
    # zoek door alle lots (ts uniek per entry in deze bot)
    i = 0
    while qty_left > 1e-9 and i < len(paper[lots_key]):
        e = paper[lots_key][i]
        if e.get("ts") != ref_ts:
            i += 1
            continue
        ...
        qty_left -= q
        if e["qty"] <= 1e-9:
            paper[lots_key].pop(i)
            # niet i++ want lijst is verschoven
        else:
            i += 1
    ...
    for o in list(paper["open_orders"]):
        ...
        if o["side"] == "sell":  # close LONG
            if trigger_long_price >= o["price"] * eps_up:
                ...
                parts1, filled1 = _take_by_ts("open_entries", o.get("ref_ts"), qty)
                rem = qty - filled1
                parts2, filled2 = _fifo_take("open_entries", rem) if rem > 1e-9 else ([], 0.0)
                parts = parts1 + parts2
        ...
        # ---- SHORT sluiting (buy-to-cover) ----
            ...
            if trigger_short_price <= o["price"] * eps_dn:
                qty = o["qty"]; price = o["price"]
                parts1, filled1 = _take_by_ts("short_entries", o.get("ref_ts"), qty)
                rem = qty - filled1
                parts2, filled2 = _fifo_take("short_entries", rem) if rem > 1e-9 else ([], 0.0)
                parts = parts1 + parts2
                filled = filled1 + filled2
                if filled <= 0:
                    o["status"] = "filled"
...
def bot_thread():
    ...
    last_rebuild_ms = int(state.get("last_rebuild_ms") or time.time()*1000)
    ...
    while not _stop.is_set():
        try:
            ...
            if not need_rebuild:
                ...
                # Recenter: als prijs/center te ver uiteen lopen binnen de band -> grid toch verversen
                if (not need_rebuild) and (RECENTER_ATR_MULT > 0) and (last_center is not None):
                    if abs(close - last_center) > (RECENTER_ATR_MULT * atr):
                        if NOTIFY_REBUILD:
                            send_telegram(f"⚠️ Recenter: |close-center|>{RECENTER_ATR_MULT:.2f} ATR — rebuild")
                        need_rebuild = True

                # Time-based refresh: voorkom 'vastplakken' aan oud bereik
                if (not need_rebuild) and (MAX_CENTER_AGE_SEC > 0):
                    if (now_ms - last_rebuild_ms) > (MAX_CENTER_AGE_SEC * 1000):
                        if NOTIFY_REBUILD:
                            send_telegram(f"⏱️ Refresh: center ouder dan {MAX_CENTER_AGE_SEC}s — rebuild")
                        need_rebuild = True
            ...
            if need_rebuild:
                ...
                save_state({
                    ...
                    "last_spacing": last_spacing, "updated_at": now_utc().isoformat(), "last_rebuild_ms": last_rebuild_ms,
