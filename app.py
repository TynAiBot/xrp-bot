...
# V2 guardrails: TV-exits pas negeren met echte winstbuffer; Trend Runner alleen activeren bij voldoende entry-beweging.
TREND_IGNORE_EXIT_MIN_PROFIT_PCT = env_float("TREND_IGNORE_EXIT_MIN_PROFIT_PCT", "0.35")
TREND_MIN_ENTRY_ATR_PCT = env_float("TREND_MIN_ENTRY_ATR_PCT", "0.12")
...
def tg_startup_msg() -> str:
    ...
    lines = [
        ...
        f"🧯 Trend guards: ignore exit vanaf +{TREND_IGNORE_EXIT_MIN_PROFIT_PCT:.2f}% | entry ATR min {TREND_MIN_ENTRY_ATR_PCT:.2f}%",
...
def entry_context(symbol: str, price: float = 0.0) -> Dict[str, Any]:
    """Korte timeframe entrykwaliteit voor Trend Runner-activatie."""
    try:
        tf = normalize_tf(SUPERVISOR_TF) or "5m"
        ohlcv = fetch_ohlcv_supervisor(symbol, tf, max(80, SUPERVISOR_OHLCV_LIMIT))
        if not ohlcv or len(ohlcv) < 80:
            return {"ok": False, "reason": f"entry_not_enough_ohlcv:{len(ohlcv) if ohlcv else 0}"}
        highs = [float(x[2]) for x in ohlcv]
        lows = [float(x[3]) for x in ohlcv]
        closes = [float(x[4]) for x in ohlcv]
        ...
        ok = atr_pct >= TREND_MIN_ENTRY_ATR_PCT
        return {
            "ok": bool(ok),
            "tf": tf,
            "atr_pct": float(atr_pct),
            ...
            "reason": f"entry_tf={tf} atr={atr_pct:.2f}% min={TREND_MIN_ENTRY_ATR_PCT:.2f}% adx={adx:.1f} rsi={rsi:.1f} vol={vol_factor:.2f}x",
        }
    except Exception as e:
        return {"ok": False, "reason": f"entry_context_error:{e}"}
...
def open_position(symbol: str, side: str, price: float, source: str, tf: str) -> Tuple[Dict, int]:
    ...
    try:
        ...
        if SIMULATE:
            ...
            entry_ctx = entry_context(symbol, entry) if TREND_RUNNER_ENABLED and trend_ctx.get("ok", False) else {"ok": True, "reason": "entry_guard_not_needed"}
            st["trend_entry_reason"] = entry_ctx.get("reason", "")
            st["trend_runner_active"] = bool(trend_ctx.get("ok", False) and entry_ctx.get("ok", False))
            st["trend_runner_reason"] = trend_ctx.get("reason", "")
            if trend_ctx.get("ok", False) and not entry_ctx.get("ok", False):
                st["trend_runner_reason"] = f"trend_ok_but_entry_guard_blocked | {trend_ctx.get('reason','')} | {entry_ctx.get('reason','')}"
            ...
            if TREND_RUNNER_ENABLED and trend_ctx.get("ok", False) and not st["trend_runner_active"] and TREND_NOTIFY:
                send_tg("\n".join([
                    "⚠️ Trend Runner NIET actief",
                    f"Side: {side.upper()} {sym_label(symbol)}",
                    "Reden: entry_guard",
                    f"Trend: {trend_ctx.get('reason','-')}",
                    f"Entry: {entry_ctx.get('reason','-')}",
                    "TV-exits worden normaal gevolgd",
                ]))
            if st["trend_runner_active"] and TREND_NOTIFY:
                send_tg("\n".join([
                    ...
                    f"Side: {side.upper()} {sym_label(symbol)}",
                    f"TF: {TREND_TF}",
                    f"Reden: {st['trend_runner_reason']}",
                    f"Entry guard: {st.get('trend_entry_reason','-')}",
                    f"Exit negeren vanaf: +{TREND_IGNORE_EXIT_MIN_PROFIT_PCT:.2f}%",
                    f"TP1: {TREND_TP1_PCT:.2f}% / {TREND_TP1_SELL_PCT:.0f}%",
                    f"Trail: {TREND_TRAIL_TRIGGER_PCT:.2f}%/{TREND_TRAIL_DISTANCE_PCT:.2f}%",
                    f"Max hold: {TREND_MAX_HOLD_HOURS:g} uur",
                ]))
        else:
...
def should_ignore_tv_exit_for_trend(symbol: str, side: str, price: float, source: str) -> Tuple[bool, str]:
    ...
    if profit_pct < TREND_IGNORE_EXIT_MIN_PROFIT_PCT:
        return False, f"profit_buffer_too_small:{profit_pct:.2f}%<{TREND_IGNORE_EXIT_MIN_PROFIT_PCT:.2f}%"
