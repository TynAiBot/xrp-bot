...
def webhook():
    ...
    if action == "buy":
        ...
        with lock:
            ...
            # 4) Order: eerst quoteOrderQty, bij 0 fill fallback naar quantity
            if LIVE_MODE and LIVE_EXCHANGE == "mexc":
                ex = _mexc_live()
                # --- primary: quoteOrderQty ---
                try:
                    # Probeer create_order met quoteOrderQty
                    spend = float(TRADE_BUDGET_LOCAL)
                    try:
                        spend_prec = float(ex.price_to_precision(SYMBOL_TV, spend))
                    except Exception:
                        spend_prec = float(round(spend, 6))
                    params = {"recvWindow": _rw, "quoteOrderQty": spend_prec, "cost": spend_prec}
                    order = ex.create_order(SYMBOL_TV, "market", "buy", None, None, params)
                    ...
                    # 0-fill? → FALLBACK met quantity
                    if pos_amount <= 0 or pos_quote <= 0:
                        _dbg("[LIVE] BUY zero-fill → fallback met quantity")
                        # prijs pakken
                        p = float(price or tv_price or 0.0)
                        if p <= 0:
                            try:
                                t = ex.fetch_ticker(SYMBOL_TV)
                                p = float(t.get("last") or t.get("close") or 0.0)
                        ...
                        if p > 0:
                            ...
                            if min_amt and qty < min_amt:
                                qty = min_amt
                            try:
                                qtyp = float(ex.amount_to_precision(SYMBOL_TV, qty))
                            except Exception:
                                qtyp = float(round(qty, 6))
                            if qtyp > 0:
                                order_fb = ex.create_order(SYMBOL_TV, "market", "buy", qtyp, None, {"recvWindow": _rw})
                                avg2b, filled2b, gross_qb, fee_qb, net_in_b = _order_quote_breakdown(ex, SYMBOL_TV, order_fb, "buy")
                                price      = float(avg2b or price or 0.0)
                                pos_amount = float(filled2b or 0.0)
                                pos_quote  = float(net_in_b or 0.0)
                                if pos_quote <= 0 and pos_amount > 0:
                                    pos_quote = round(price * pos_amount, 6)
                                _dbg(f"[LIVE] Fallback BUY {SYMBOL_TV} id={order_fb.get('id')} filled={pos_amount} avg={price} gross={gross_qb} fee_q={fee_qb} pos_quote={pos_quote}")

                    # Na fallback nog steeds 0? → netjes stoppen
                    if pos_amount <= 0 or pos_quote <= 0:
                        _dbg("[LIVE] BUY still zero after fallback → skip")
                        commit(SYMBOL_TV)
                        return "OK", 200
